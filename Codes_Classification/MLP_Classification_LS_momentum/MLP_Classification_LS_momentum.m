...
% Modification: The weight updates are controlled by introducing a momentum
% term while updating weights.
...
% values to hold old DeltaW data to use in momentum term
DWiOld = zeros(hid,inp);
DWoOld = zeros(out,hid);
...
for ep = 1 : epo
    ...
    for sa = 1 : NTD
        ...
        er = tt - Yo;               % Error
        DWo = DWo + lam * (er * Yh') + alpha * DWoOld;   % including the momentum term                % update rule for output weight
...
% matrix containing actual output and the corresponding predicted output
predict_tes = zeros(NTD,2);

% loop over samples for prediction
for sa = 1: NTD
...
disp('testing')
disp(sqrt(rmstes/NTD)) % mean squared error
